...
void ST7735_Line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
	...
	/* First we compute the slope and intercept and check for vertical
		 or horizontal lines, since those are easy to draw
		 Otherwise, we draw the line pixel-by-pixel */
									 
	float slope = 0; 				// Slope
	int32_t intercept = 0; 	// Y-Intercept
	int32_t dy = 0;					// Delta x and y
	int32_t dx = 0;
	int32_t xpos = 0;				// Hold current draw position
  int32_t ypos = 0;									 
	int32_t xStart = 0;			// Hold starting draw position
	int32_t xEnd = 0;				// And ending draw position
	int32_t yStart = 0;
	int32_t yEnd = 0; 
	
	dy = y1-y2;
	dx = x1-x2;
	slope = ((float) dy)/dx;			 // Compute slope
	intercept = y1 - (slope * x1); // Compute offset

	if(x1 == x2) {				// Vertical line
		if(y1 < y2) {				// Align so we draw from bottom to top
			yStart = y1;
			yEnd = y2;
			xpos = x1; 
		} else {
		...
		// Pass it off to Vertical FastDraw Function
		ST7735_DrawFastVLine(xpos, yStart, yEnd - yStart, color);
	}	else if(y1 == y2) {	// Horizontal line
		if(x1 < x2) {				// Align so we draw from left to right
			xStart = x1;
			xEnd = x2;
			ypos = y1; 
		} else {
		...
		// Pass it off to Horizontal FastDraw Function
		ST7735_DrawFastHLine(xStart, ypos, xEnd - xStart, color);
	}	else { 							// General case where 0 < slope < infinity
		if(x1 < x2){  			// Left-to-right drawing
			xStart = x1; 
			xEnd = x2; 
		} else { 
			xStart = x2; 
		...
		if(abs(x2 - x1) > abs(y2 - y1)) {
			for(xpos = xStart; xpos <= xEnd; xpos++) {
				...
				ST7735_DrawPixel(xpos, ypos, color); 
			}	
		} else {
